...
void IndexFile::split(DiskBlock& parent, DiskBlock& child, int32_t parentOffset, int32_t childOffset)
{
    ...
    std::vector<Field> blockFields{ child.m_records[0].m_data[0], Field::asInteger(), Field::asInteger() };
...
void IndexFile::insertNonFull(DiskBlock& block, int32_t blockOffset, Field field, int32_t dataBlockIndex)
{
    ...
    if (isLeaf(block)) {
        ...
        //child is full
        if (child.m_records.size() == m_order) {
...
void IndexFile::insert(Field field, int32_t dataBlockIndex)
{
    ...
    if (m_locatedBlocks == 0) {
        ...
        //root is full
        if (rootBlock.m_records.size() == m_order) {
